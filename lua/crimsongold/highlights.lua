...
function M.apply(p, opts)
  ...
  local plugins = {
    ...
    SnacksPickerNormal = { fg = p.fg, bg = p.bg_float },
    SnacksPickerBorder = { fg = p.gold_dim, bg = p.bg_float },
    SnacksPickerTitle = { fg = p.gold, bg = p.bg_float, style = "bold" },
    SnacksPickerPreview = { fg = p.fg, bg = p.bg_float },
    SnacksPickerPreviewBorder = { fg = p.gold_dim, bg = p.bg_float },
    SnacksPickerPreviewTitle = { fg = p.gold, bg = p.bg_float, style = "bold" },
    SnacksPickerPrompt = { fg = p.gold },
    SnacksPickerInput = { fg = p.fg_bright },
    SnacksPickerHeader = { fg = p.gold, style = "bold" },
    SnacksPickerListCursorLine = { bg = p.bg_select },
    SnacksPickerCurrent = { fg = p.fg_bright },
    SnacksPickerFile = { fg = p.fg },
    SnacksPickerDir = { fg = p.fg_dim },
    SnacksPickerUnselected = { fg = p.fg_dim },
    SnacksPickerPathHidden = { fg = p.fg_faint },
    SnacksPickerPathIgnored = { fg = p.fg_faint },
    SnacksPickerDimmed = { fg = p.fg_faint },
    SnacksPickerMatch = { fg = p.gold_bright },

    LazyH1 = { fg = p.gold, style = "bold" },
    LazyH2 = { fg = p.gold_dim, style = "bold" },
    LazyH3 = { fg = p.orange, style = "bold" },
    LazyComment = { fg = p.fg_dim, style = "italic" },
    LazyDimmed = { fg = p.fg_faint },
    LazyTaskOutput = { fg = p.fg_dim },
    LazyLocal = { fg = p.gold },
    LazyButton = { fg = p.fg_dim, bg = p.bg_light },
    LazyButtonActive = { fg = p.fg_on_select, bg = p.bg_select },
